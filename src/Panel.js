...
function Panel({visibility, onClick, data, coords, panToPlace}) {
	return (
      ...
      <div id="Panel" className={visibility}>
        <div className="Panel__PlaceList">
          {data.length > 0 ? data.map(place => (
            <PlaceDisplay key={place.key} address={place.address} name={place.name} rating={place.rating} lat={place.latlng.lat} lng={place.latlng.lng} coords={coords} panToPlace={panToPlace}/>
